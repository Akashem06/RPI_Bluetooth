...
GATTError GATT_send_notification(uint16_t connection_handle, uint16_t char_handle, uint8_t *value,
  ...
  HCIAsyncData acl_data = { .connection_handle = connection_handle,
                            .pb_flag = 0,
                            .bc_flag = 0,
                            .data_total_length = length + 3,
                            .data = packet };

  return (HCI_send_async_data(&acl_data) == HCI_ERROR_SUCCESS) ? GATT_ERROR_SUCCESS
                                                               : GATT_ERROR_INSUFFICIENT_RESOURCES;
}