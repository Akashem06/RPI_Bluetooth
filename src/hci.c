...
uint8_t *bcm4345c0_fw_ptr = _binary_BCM4345C0_hcd_start;
uint8_t *bcm4345c0_fw_end = _binary_BCM4345C0_hcd_end;
...
void HCI_handle_command_status_event(uint8_t *parameters, uint8_t parameter_length) {
    ...
    log("Handling command status event!\n\r");
    switch (op_code) {
...
void HCI_handle_hw_rx(uint8_t byte) {
    if(rx_count >= MAX_PACKET_SIZE) return;
...
HCIError HCI_BLE_set_advertising_param(uint16_t adv_interval_min, uint16_t adv_interval_max, 
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_SET_ADVERTISING_PARAMETERS,
...
HCIError HCI_BLE_set_advertising_data(uint8_t *adv_data, uint8_t adv_data_len) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_SET_ADVERTISING_DATA,
...
HCIError HCI_BLE_set_scan_parameters(Scan_Type scan_type, uint16_t scan_interval_ms, uint16_t scan_window_ms,
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_SET_SCAN_PARAMETERS,
...
HCIError HCI_BLE_set_scan_enable(bool enable, bool filter_duplicates) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_SET_SCAN_ENABLE,
        .parameter_length = sizeof(params),
        .parameters = params
    };
...
HCIError HCI_BLE_create_connection(uint16_t scan_interval_ms, uint16_t scan_window_ms,
    ...
    uint8_t params[25] = {
        ...
        filter_policy,
        peer_address_type,
        peer_address[0], peer_address[1], peer_address[2],    /* Peer address */
        ...
        conn_latency & 0xFF, (conn_latency >> 8) & 0xFF,
        supervision_timeout_ms & 0xFF, (supervision_timeout_ms >> 8) & 0xFF,
        0x00, 0x00,  /* Minimum CE length */
        0x00, 0x00   /* Maximum CE length */
    };

    HCICommand cmd = {
        .op_code.raw = CMD_BLE_CREATE_CONNECTION,
        .parameter_length = sizeof(params),
        .parameters = params
    };
...
HCIError HCI_BLE_connection_update(uint16_t connection_handle,
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_CONNECTION_UPDATE,
        .parameter_length = sizeof(params),
        .parameters = params
    };
...
HCIError HCI_BLE_set_event_mask(uint8_t mask) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BLE_SET_EVENT_MASK,
        .parameter_length = sizeof(params),
        .parameters = params
    };
...
HCIError HCI_set_local_name(const char* name) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BT_WRITE_LOCAL_NAME,
        .parameter_length = sizeof(params),
        .parameters = params
    };
...
HCIError HCI_bcm4345_load_firmware(void) {
    ...
    // Validate firmware
    if (bcm4345c0_fw_size != (bcm4345c0_fw_end - bcm4345c0_fw_ptr)) {
    ...
    while (bcm4345c0_fw_ptr < bcm4345c0_fw_end) {
        ...
        log("fw_op_code: %d\n\r", fw_op_code);
        log("fw_parameter_length: %d\n\n\r", fw_parameter_length);

        if (fw_parameter_length > 255 || bcm4345c0_fw_ptr + fw_parameter_length > bcm4345c0_fw_end) {
            return HCI_ERROR_BUFFER_OVERFLOW;
        }
...
HCIError HCI_get_module_status(BCM4345C0Info *info) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BT_READ_LOCAL_VERSION_INFORMATION,
        .parameter_length = 0,
        .parameters = NULL
    };
...
HCIError HCI_get_bt_addr(uint8_t *bt_addr) {
    ...
    HCICommand cmd = {
        .op_code.raw = CMD_BT_READ_BD_ADDR,
        .parameter_length = 0,
        .parameters = NULL
    };
...
HCIError HCI_init(void) {
    ...
    if (status != HCI_ERROR_SUCCESS) {
        log("Failed load firmwre %d\r\n", status);
        return status;
    }
    hw_delay_ms(1000);
    hci_state = HCI_STATE_ON;

    /* Wait for chip to process firmware.*/
    // hw_delay_ms(0x100000);

    /* Update the baudrate. OPTIONAL */
    // status = HCI_bcm4345_set_baudrate(115200);
